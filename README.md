...
화면 왼쪽에 서울 기준 PPI 레이더가 상시 표시됩니다. 회원국 20개국을 실제
좌표에서 계산한 대권 방위·거리에 찍고, 주사선이 4초마다 한 바퀴 돌며 소인을
훑습니다. 소인은 방위에 맞춰 밝아졌다 어두워져 주사선이 지날 때 칠해지는 것처럼
보입니다. 거리는 제곱근 축척인데, 서울 반경 1만km 안에 대부분이 몰려 있어
선형으로 그리면 안쪽에 뭉쳐 읽을 수 없기 때문입니다. 주의 단계 이상만 부호를
답니다.

### 자동 파견

**조작할 것이 없습니다.** 지휘부가 상황을 보고 스스로 파견을 발령합니다.
우선순위는 대응제 완성 → 최악 피해국 구호 → 이동 통제선 → 회선 복구 순이고,
대응제가 미완성인 동안에는 연구단 몫의 지휘 역량을 항상 남겨 둡니다.

발령되는 파견은 `연구단 증원 · 신속대응단 파견 · 봉쇄선 설정 · 통신중계 복구 ·
광역 통제 발령` 다섯 가지입니다. 각 파견에는 실제 부대가 붙어서 전문에 편성과
행선지가 함께 기록되고, 파견 현황 패널에서 전개중 → 임무중 → 복귀 순으로
상태가 바뀝니다.

최초 파견까지 동원에 8초가 걸립니다. 그 사이에 미국에서 시작한 전파가 자라는
것을 그대로 보게 됩니다.
...
**승리** — 대응제 100%를 감염률 50% 미만에서 달성하면 복구 순서가 진행되고
콘솔이 통상 근무로 돌아옵니다. **패배** — 감염률 92%에서 본 단말 반경 1,000미터
접촉, 방폭문 순차 돌파, 최종 데이터 보존 단계로 이어집니다. 재시작 수단은 없고,
다시 하려면 새로고침합니다.

억제만으로는 끝나지 않습니다. 대응제가 미완성인 동안 제거되지 않은 병원소가
다시 터지는데, 무작위가 아니라 이미 발생했던 국가와 연결된 곳에서 터지므로
미국에서 시작해 번져나간 흐름이 유지됩니다.

난이도는 **반드시 이기되 사태가 눈에 보이도록** 맞췄습니다. 헤드리스 하네스로
확인한 자동 운용 곡선은 다음과 같습니다.
...
| T+60초 | 주변국 확산 진행 |
| T+120초 | 여러 국가 교전 단계 |
| T+196초 | 대응제 100% 달성 · 판정 |

경고는 **최소 12초 간격**을 두고 발령됩니다. 원래 후반부에 3~4초까지 좁아져
연달아 터지던 구간을 약 3배로 벌린 값이고, 위기 지수가 완만한 구간의 간격
(14·16·24초)은 그대로 둡니다. 한 판에서 약 14건이 발령됩니다. 나머지
경고는 상황이 더 나빠지는 경우를 위해 남겨 둡니다. 경고 발령은 평균 감염률만으로는
잘 안 잡혀서 — 지휘가 자동으로 억제하니 평균이 낮게 유지됩니다 — 최악 피해국과
상실 노드 수까지 함께 보는 위기 지수로 판정합니다.
...
assets/js/scenario-data.js  이동 경로, 전문 문안, 경고 대본, 파견 부대
assets/js/scenario.js       게이트, 경고 대기열, 전파 모형, 자동 파견, 레이더,
                            노드 붕괴, 결말
```
